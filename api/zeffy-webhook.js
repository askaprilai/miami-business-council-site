/**
 * Miami Business Council - Zeffy Webhook Handler
 * Receives member signup data from Zeffy via Zapier and creates member accounts
 *
 * This endpoint is called by Zapier when a new membership payment is received in Zeffy
 *
 * Expected data from Zapier/Zeffy:
 * - firstName, lastName, email, phone
 * - membershipType: 'individual', 'nonprofit', or 'business'
 * - billingFrequency: 'monthly' or 'annual'
 * - amount: payment amount
 * - zeffyTransactionId: unique transaction ID from Zeffy
 */
...
export default async function handler(req, res) {
  ...
  try {
    console.log('📨 Zeffy webhook received:', JSON.stringify(req.body, null, 2));
    ...
    const {
      ...
      zeffyTransactionId,
      zeffyCustomerId,
      // Additional fields from Zeffy form
      industry,
      employeeCount,
      incorporated,
      interests
    ...
    const validMembershipTypes = ['individual', 'nonprofit', 'business'];
    if (membershipType && !validMembershipTypes.includes(membershipType.toLowerCase())) {
      console.error('❌ Invalid membership type:', membershipType);
      return res.status(400).json({
        success: false,
        error: 'Invalid membership type. Must be: individual, nonprofit, or business'
      });
    }

    // Validate billing frequency
    const validBillingFrequencies = ['monthly', 'annual'];
    if (billingFrequency && !validBillingFrequencies.includes(billingFrequency.toLowerCase())) {
      console.error('❌ Invalid billing frequency:', billingFrequency);
      return res.status(400).json({
        success: false,
        error: 'Invalid billing frequency. Must be: monthly or annual'
      });
    }
    ...
    console.log(`📧 Processing Zeffy payment for: ${email}`);
    ...
    const { data: authData, error: authError } = await supabaseAdmin.auth.admin.createUser({
      ...
      user_metadata: {
        ...
        membership_type: membershipType?.toLowerCase() || 'individual',
        billing_frequency: billingFrequency?.toLowerCase() || 'annual'
      }
    });
    ...
    // Add Zeffy payment tracking
    if (zeffyTransactionId) {
      memberData.zeffy_transaction_id = zeffyTransactionId;
    }
    if (zeffyCustomerId) {
      memberData.zeffy_customer_id = zeffyCustomerId;
    }
    ...
    console.log(`  ✅ Member record created/updated: ${member.id}`);

    // Step 3: Generate magic link for member portal access
    const redirectUrl = 'https://miamibusinesscouncil.com/portal.html';
    ...
    const billingText = billingFrequency === 'monthly'
      ? `monthly billing of $${amount}`
      : `annual billing of $${amount}`;
      ...
      <body style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #ffffff; padding: 40px 30px; border-radius: 0 0 10px 10px; box-shadow: 0 4px 20px rgba(0,0,0,0.1);">
          ...
          <p style="font-size: 16px; line-height: 1.8; margin-bottom: 25px;">
            Welcome to <strong>Miami Business Council</strong>! Your ${billingFrequency} membership (${membershipType}) has been activated. Thank you for joining our community of innovative business leaders in Miami.
          </p>

          <div style="background: #f8f9fa; border-left: 4px solid #d4af37; padding: 15px 20px; margin: 25px 0;">
            <p style="margin: 0; font-size: 14px; color: #666;">
              <strong>Your Membership:</strong> ${membershipType.charAt(0).toUpperCase() + membershipType.slice(1)}<br>
              <strong>Billing:</strong> ${billingText}
            </p>
          </div>
          ...
          <div style="background: #f8f9fa; border-radius: 8px; padding: 20px; margin: 30px 0;">
            ...
            <ul style="margin: 0; padding-left: 20px;">
              ...
              <li style="margin-bottom: 10px;">Explore collaboration opportunities</li>
            </ul>
          </div>
